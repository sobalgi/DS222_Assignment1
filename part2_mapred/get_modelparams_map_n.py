...
# unit test code
# cat unit_train.txt | python get_classwordcount_map.py | LANG=C sort > out_get_classwordcount_map.txt
...
for line in sys.stdin:
    ...
    # send key value as STDOUT. key = (classlabel) , value = (count 1)
    for classlabel in classlabels_list:
        print('01lbl %s\t1' % (classlabel))  # emit key-val for each class to find the class counts
        #print(f'{classlabel}\t1')  # emit key-val for each class to find the class counts

        # send key value as STDOUT. key = (classlabel word) , value = (count 1)
        for word in filtered_words:
            #classlabel_word_key = classlabel + ' ' + word  # send 1 for per word per classlabel in the document
            #print(f'{classlabel_word_key{\t1')
            print('02wrd %s %s\t1' % (classlabel, word))
