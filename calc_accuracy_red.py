...
accuracy_count = 0
total_doc_count = 0
label_list = []  # dummy initialization
...
for line in sys.stdin:
    ...
    if current_doc_id == '-prior':
        prior_prob_dict[current_label] = float(val)  # add to prior dictionary
        if prior_prob_dict[current_label] >= 0:
            raise ValueError
        ...
        if previous_doc_id == current_doc_id and previous_label == current_label:
            ...
            post_prob_dict = prior_prob_dict.copy()
            total_cond_prob = float(current_post_prob)
            previous_label = current_label
...
max_label_ix = np.argmax(list(post_prob_dict.values()))
predicted_label = label_list[max_label_ix]
currentlabels_list = current_labels.split(',')  # get class labels as list

if predicted_label in currentlabels_list:
    accuracy_count += 1
